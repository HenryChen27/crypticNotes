...
支持本地记录成功与失败案例。在设置中开启「记录识别案例」，通过「打开记录」找到截图、程序版本、触发原因和诊断数据；合计最多保留最近 20 条，超出自动删除最早的记录。协作者可将整个目录压缩后发送给开发者。记录不会自动上传，分享前请检查截图内容。详见 [案例收集说明](docs/failure-records.md)。
